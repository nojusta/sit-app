...
export async function uploadProfilePicture(file) {
  try {
    ...
    // Get the file URL
    const fileUrl = client.getFileView(appwriteConfig.storageId, response.$id);
...
// Function to store marker information
export async function storeMarker(marker) {
  try {
    const response = await databases.createDocument(
      appwriteConfig.databaseId,
      appwriteConfig.markersCollectionId,
      ID.unique(),
      marker
    );
    return response;
